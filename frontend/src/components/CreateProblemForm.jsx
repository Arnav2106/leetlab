...
const CreateProblemForm = () => {
  ...
  const handleImportJson = () => {
    ...
    try {
      ...
      // Basic validation
      if (!parsed.title) throw new Error("Missing required field: title");
      if (!parsed.description) throw new Error("Missing required field: description");
      if (!parsed.testcases || !Array.isArray(parsed.testcases))
        throw new Error("Missing or invalid: testcases (must be array)");
      if (!parsed.codeSnippets) throw new Error("Missing field: codeSnippets");
      if (!parsed.referenceSolutions) throw new Error("Missing field: referenceSolutions");
      // Load into form
      if (parsed.tags) replaceTags(parsed.tags.map((t) => t));
      if (parsed.testcases) replacetestcases(parsed.testcases.map((tc) => tc));
      reset(parsed);
      setShowImport(false);
      setImportJson("");
        ...
        <div className="card-body p-6 md:p-8">
          ...
          {showImport && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
              <div className="card bg-base-100 shadow-2xl w-full max-w-2xl mx-4">
                <div className="card-body">
                  <h3 className="card-title text-xl mb-1">Import Problem from JSON</h3>
                  <p className="text-sm text-base-content/60 mb-4">
                    Paste a valid problem JSON below. Must include: <code className="text-primary">title</code>,{" "}
                    <code className="text-primary">description</code>,{" "}
                    <code className="text-primary">testcases</code>,{" "}
                    <code className="text-primary">codeSnippets</code>,{" "}
                    <code className="text-primary">referenceSolutions</code>.
                  </p>
                  <textarea
                    className="textarea textarea-bordered w-full font-mono text-sm"
                    rows={14}
                    placeholder='{"title":"Two Sum","description":"...","difficulty":"EASY","tags":["arrays"],...}'
                    value={importJson}
